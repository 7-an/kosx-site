...
- 本地预览有两条受支持路径：直接双击根目录 `index.html` 时，`file://` 专用逻辑补齐物理 `*/index.html` 链接并加载粒子 IIFE 兼容包；`npm run dev` 时访问 `http://127.0.0.1:8765`，核对与线上一致的干净路径。

## 信息架构（7 个视图）

| 路由 | 名称 | 内容要点 |
|---|---|---|
| `/` | 首页 | Hero（620px 居中：中英切换标题 + 副文案 + Explore 下滑按钮，背景留空）→ 合作伙伴信任带（流动、无编号）→ 行动页（断言 + 探索项目/加入团队双大卡）→ 品牌素材预留页（留空）→ 01 我们是谁（头像簇 + Built by + 品牌流动带 + Mission/Vision + 媒体 bento + 活动流动带）→ 02 我们如何创造价值（bento：三入口 + 橙色加入 CTA，素材槽留空）|
...
- 序列：纯黑场（#000，无网格无光晕）→ KOSX Logo 单独在正中央淡入、停留约 1 秒 → Logo 直接飞向左上角成为页首 Logo → 数万颗粒子从右侧飘入聚合成「BUILD SOMETHING REAL.」（三行下行阶梯错位：BUILD 居左、SOMETHING 居中、REAL. 居右；粒子画面中不再有 Logo）→ **短暂驻留**：粒子定型后完全静止（仅指针扰动回弹），底部双箭头持续脉动 → 最迟在开屏开始 5 秒后自动淡出到 Hero。
- 进入方式：无 Skip 按钮；访客可在 5 秒内通过滚轮下滑、触摸上滑、点击双箭头或键盘提前进入；没有操作时由 5 秒计时自动进入。
...
- 只在进入首页时播放；六个子路径直接显示内容，不播放开屏。首页开屏最多停留 5 秒，手动提前进入与自动进入共用同一淡出转场。
